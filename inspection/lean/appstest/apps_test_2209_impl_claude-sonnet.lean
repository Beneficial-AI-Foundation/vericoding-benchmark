...
-- <vc-helpers>
-- LLM HELPER: Extract strings from input[1] to input[n]
def extractStrings (input : List String) (n : Nat) : List String :=
  (List.range n).map (fun i => input[i + 1]!)

-- LLM HELPER: Sort strings by their StringRatio
def sortByRatio (strings : List String) : List String :=
  strings.mergeSort (fun s1 s2 => StringRatio s1 ≤ StringRatio s2)
-- </vc-helpers>

-- <vc-definitions>
def solve (input : List String) (h_precond : solve_precond input) : Int :=
  let n := StringToInt input[0]!
  let strings := extractStrings input n
  let sorted := sortByRatio strings  
  let concatenated := ConcatenateStrings sorted
  Int.ofNat (CountShSubsequences concatenated)
-- </vc-definitions>
